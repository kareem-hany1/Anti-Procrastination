<x-layout>
    <x-slot:title>Tache | Creer</x-slot:title>
    <div class="layout-container flex h-full grow flex-col">
        <x-header/>
        <div class="px-40 flex justify-center py-5">
            <form method="post" action="/step/{{request()->route('project')}}/create" class=" flex mx-auto flex-col w-[512px] max-w-[512px] py-5 max-w-[960px]">
                @csrf
                <div class="flex flex-wrap justify-between gap-3 p-4"><p class="text-white tracking-light text-[32px] font-bold leading-tight min-w-72">Ajouter Une Tache</p></div>
                ...
                <div class="flex max-w-[480px] flex-wrap items-end gap-4 px-4 py-3">
                    <label class="flex flex-col min-w-40 flex-1">
                        ...
                        >
                            <option selected="{{old('priority') == 'normal'}}" value="normal">Normal</option>
                            <option selected="{{old('priority') == 'medium'}}" value="medium">Moyen</option>
                            <option selected="{{old('priority') == 'high'}}" value="high">Haut</option>
                        </select>
                        @error('priority')
                        {{$message}}
                        @enderror
