<div class="px-4 md:px-10 lg:px-40 flex justify-center py-5 w-full">
    <div class="layout-content-container flex flex-col w-full">
        ...
        @endphp
        <div class="pb-3">
            <div class="flex border-b border-[#324d67] px-4 gap-4 md:gap-8 overflow-x-auto">
                <button wire:click="filterMenu('{{$menus[0]}}')" class="flex flex-col items-center justify-center {{$filter=='all'?'border-b-[3px] border-b-[#2b8dee] text-white':'text-[#92adc9]'}} pb-[13px] pt-4 whitespace-nowrap">
                    <p class="{{$filter=='all'?'text-white':'text-[#92adc9]'}} text-xs md:text-sm font-bold leading-normal tracking-[0.015em]">Toutes ({{$all}})</p>
                </button>
                <button wire:click="filterMenu('{{$menus[1]}}')" class="flex flex-col items-center justify-center {{$filter=='todo'?'border-b-[3px] border-b-[#2b8dee] text-white':'text-[#92adc9]'}} pb-[13px] pt-4 whitespace-nowrap">
                    <p class="{{$filter=='todo'?'text-white':'text-[#92adc9]'}} text-xs md:text-sm font-bold leading-normal tracking-[0.015em]">A Faire ({{$todo}})</p>
                </button>
                <button wire:click="filterMenu('{{$menus[2]}}')" class="flex flex-col items-center justify-center {{$filter=='pending'?'border-b-[3px] border-b-[#2b8dee] text-white':'text-[#92adc9]'}} pb-[13px] pt-4 whitespace-nowrap">
                    <p class="{{$filter=='pending'?'text-white':'text-[#92adc9]'}} text-xs md:text-sm font-bold leading-normal tracking-[0.015em]">En Cours ({{$pending}})</p>
                </button>
                <button wire:click="filterMenu('{{$menus[3]}}')" class="flex flex-col items-center justify-center {{$filter!=='completed'?'text-[#92adc9]':'border-b-[3px] border-b-[#2b8dee] text-white'}} pb-[13px] pt-4 whitespace-nowrap">
                    <p class="{{$filter!=='completed'?'text-[#92adc9]':'text-white'}} text-xs md:text-sm font-bold leading-normal tracking-[0.015em]">Terminée ({{$completed}})</p>
                </button>
            </div>
        </div>
        ...
        <div class="flex gap-3 p-3 flex-wrap pr-4">
            <button wire:click="dues_date" class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-[#233648] pl-4 pr-2">
                ...
                <p class="text-white text-xs md:text-sm font-medium leading-normal">Date Limite</p>
            ...
            <button wire:click="priorit" class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-[#233648] pl-4 pr-2">
                <svg width="15px" height="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M21 6H19M21 12H16M21 18H16M7 20V13.5612C7 13.3532 7 13.2492 6.97958 13.1497C6.96147 13.0615 6.93151 12.9761 6.89052 12.8958C6.84431 12.8054 6.77934 12.7242 6.64939 12.5617L3.35061 8.43826C3.22066 8.27583 3.15569 8.19461 3.10948 8.10417C3.06849 8.02393 3.03853 7.93852 3.02042 7.85026C3 7.75078 3 7.64677 3 7.43875V5.6C3 5.03995 3 4.75992 3.10899 4.54601C3.20487 4.35785 3.35785 4.20487 3.54601 4.10899C3.75992 4 4.03995 4 4.6 4H13.4C13.9601 4 14.2401 4 14.454 4.10899C14.6422 4.20487 14.7951 4.35785 14.891 4.54601C15 4.75992 15 5.03995 15 5.6V7.43875C15 7.64677 15 7.75078 14.9796 7.85026C14.9615 7.93852 14.9315 8.02393 14.8905 8.10417C14.8443 8.19461 14.7793 8.27583 14.6494 8.43826L11.3506 12.5617C11.2207 12.7242 11.1557 12.8054 11.1095 12.8958C11.0685 12.9761 11.0385 13.0615 11.0204 13.1497C11 13.2492 11 13.3532 11 13.5612V17L7 20Z" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                <p class="text-white text-xs md:text-sm font-medium leading-normal">Priorité</p>
                <div class="text-white" data-icon="CaretDown" data-size="20px" data-weight="regular">
                    @if($priority == 'low')
                        <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" fill="currentColor" viewBox="0 0 256 256">
                            <path d="M213.66,101.66l-80,80a8,8,0,0,1-11.32,0l-80-80A8,8,0,0,1,53.66,90.34L128,164.69l74.34-74.35a8,8,0,0,1,11.32,11.32Z"></path>
                ...
                <table class="w-full">
                    ...
                    @foreach($tasks as $task)
                        <tr class="border-t border-t-[#324d67]">
                            ...
                            <td class="table-14759d78-e315-4e2e-8b8d-9474bae781df-column-360 h-[72px] {{$task->due_date->diffInDay(now()) <= 2?'text-red-500':''}} px-4 py-2 w-[400px] text-[#92adc9] text-sm font-normal leading-normal">
                        ...
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div wire:key="task-menu-mobile-{{ $task->id }}" x-data="{ open: @entangle('openMenus.'.$task->id).live }" class="relative ml-2">
                                ...
                                <div
                                    ...
                                    x-on:click="open = false"
                                >
                                    <a href="/task/{{$task->id}}/edit" class="block w-full text-left px-4 py-2 text-sm text-white hover:bg-[#22364a]">
                                        Modifier
                                    </a>
